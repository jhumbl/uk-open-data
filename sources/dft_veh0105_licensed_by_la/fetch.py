"""VEH0105: Licensed vehicles at the end of the quarter, by body type, fuel
type, keepership and local authority. Published by DfT/DVLA.

This is the all-vehicles companion to VEH0132 (ULEVs only): same publisher,
same landing page, same quarterly cadence and local-authority geography, but
it adds a body-type dimension and covers every vehicle — so it is the
denominator you need to turn VEH0132's ULEV counts into a share, and it
carries the wider fleet fuel mix.

Two things differ from VEH0132 and shape the tidy step below:
  - an extra ``BodyType`` id column, and
  - the values are published in THOUSANDS as decimals (e.g. 109.441), not as
    integer counts — hence the derived column is ``count_thousands`` (a float),
    not ``count``.
...
page for the current VEH0105 link, then download it.

We publish two files: the unmodified ODS as downloaded (the archival record)
and a long/tidy parquet — one row per geography x body type x fuel x
keepership x quarter — whose schema is declared in datapackage.json (the
serving artifact).
...
# Matches e.g. https://assets.publishing.service.gov.uk/.../veh0105.ods
ODS_LINK_RE = re.compile(
    r'href="(https://assets\.publishing\.service\.gov\.uk/[^"]*veh0105[^"]*\.ods)"',
    re.IGNORECASE,
)

ODS_NAME = "dft_veh0105_licensed_by_la.ods"
PARQUET_NAME = "dft_veh0105_licensed_by_la.parquet"

# Wide-sheet header -> tidy column name (matched case-insensitively). The
# constant "Units" column ("Thousands") is intentionally absent — it is dropped.
ID_COLUMNS = {
    "ons code": "ons_code",
...
def find_ods_url() -> str:
    ...
    if not match:
        raise RuntimeError(
            f"Could not find a VEH0105 .ods link on {LANDING_PAGE}. "
...
def tidy_to_parquet(ods_path: Path, parquet_path: Path) -> Path | None:
    ...
    try:
        ...
        candidates = [n for n in sheets if "0105" in n] or list(sheets)
        ...
        HEADER_LABELS = {
            "ons code", "ons geography", "ons sort", "bodytype", "fuel",
        ...
        for col in ("ons_code", "ons_geography", "body_type", "fuel", "keepership"):
        ...
        # silently publishing nulls. Values are decimals in thousands.
        suppressed = df["count_thousands"].astype(str).str.strip() == "[x]"
        df["count_thousands"] = df["count_thousands"].mask(suppressed).astype("float64")
        df["ons_sort"] = df["ons_sort"].astype("Int64")

        fields = load_resource_schema(Path(__file__).parent, PARQUET_NAME)
        df = df.sort_values(
            ["ons_sort", "ons_code", "body_type", "fuel", "keepership", "quarter"],
            ignore_index=True,
...
def validate(files: list[Path]) -> None:
    ...
    if not (5_000_000 < size < 80_000_000):
        raise ValueError(
            f"{ods.name} is {size:,} bytes — outside the plausible range for "
            "VEH0105 (~17 MB). Publisher may have changed or truncated the file."
        )
    ...
    if parquet is not None:
        ...
        # ~40,700 wide rows x ~65 quarters ≈ 2.64M; leave headroom for growth.
        if not (1_500_000 < len(df) < 6_000_000):
            raise ValueError(
        ...
        if not (4 <= df["body_type"].nunique() <= 15):
            raise ValueError(
                f"{parquet.name} has {df['body_type'].nunique()} body_type "
                "values — implausible; review tidy_to_parquet()."
            )
        if not (3 <= df["fuel"].nunique() <= 15):
            raise ValueError(
        ...
        null_fraction = df["count_thousands"].isna().mean()
